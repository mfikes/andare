...
(defrecord PushTry [catch-block]
  ...
  IEmittableInstruction
  (emit-instruction [this state-sym]
    `[~'_ (cljs.core.async.impl.ioc-macros/aset-all! ~state-sym ~EXCEPTION-FRAMES (cons ~catch-block (aget ~state-sym ~EXCEPTION-FRAMES)))]))
...
(defrecord PopTry []
  ...
  IEmittableInstruction
  (emit-instruction [this state-sym]
    `[~'_ (cljs.core.async.impl.ioc-macros/aset-all! ~state-sym ~EXCEPTION-FRAMES (rest (aget ~state-sym ~EXCEPTION-FRAMES)))]))
      ...
      `(let [~ex (aget ~state-sym ~VALUE-IDX)]
         ...
         (cond
           ~@(for [[handler-idx type] catches
                   i [(if (= type :default)
                        `true
                        `(instance? ~type ~ex)) ` (cljs.core.async.impl.ioc-macros/aset-all! ~state-sym
          ...
          (let [ret-value# (try (loop []
                                  (let [result# (switch# ~state-sym)]
                                    (if (cljs.core/keyword-identical? result# :recur)
                                      ...
                                      result#)))
                                (catch :default ex#
                                  (cljs.core.async.impl.ioc-macros/aset-all! ~state-sym ~VALUE-IDX ex#)
